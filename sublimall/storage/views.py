...
class UploadPackageAPIView(APIMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        old_packages = member.package_set.filter(version=version).exclude(pk=package.pk)
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if package_id:
                ...
                package = (
                    member.package_set.filter(version=version)
                    .only("package")
                    .latest("update")
                )
...
class MaxPackageSizeAPIView(APIMixin, View):
    def post(self, request):
        ...
        return HttpResponse(
            json.dumps({"success": True, "output": member.get_storage_limit()})
        )
